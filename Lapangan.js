...
class Lapangan extends Component {
    ...
    Drop = (id) => {
        if (window.confirm("Apakah anda yakin ingin menghapus data ini?")) {
            $("#loading").toast("show");
            let url = "http://localhost:8080/lapangan/public/lapangan/drop/" + id;
            axios.delete(url)
            .then(response => {
                $("#loading").toast("hide");
    ...
    Save = (event) => {
        ...
        let url = "http://localhost:8080/lapangan/public/lapangan/save";
    ...
    search = (event) => {
        if (event.keyCode === 13) {
            $("#loading").toast("show");
            let url = "http://localhost:8080/lapangan/public/lapangan/find";
            let form = new FormData();
                                ...
                                { this.state.lapangan.map((item) => {
                                    return(
                                        <tr key={item.id}>
                                            <td>{item.nama}</td>
                                            <td>{item.harga}</td>
                                            <td><img src={'http://localhost:8080/lapangan/public/gambar/' + item.gambar}
